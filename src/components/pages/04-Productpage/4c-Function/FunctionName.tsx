...
const FunctionName:FC<FunctionProps> = ({functionDetail, locale, index}) => {
    const {title, image, slug} = functionDetail;
    const getPath = (locale: string, slug: string) => {
        if (locale === 'en') {
          return `/en/product/function/${slug}`;
        } else if (locale === 'vn') {
          return `/vn/san-pham/chuc-nang/${slug}`;
        } else {
