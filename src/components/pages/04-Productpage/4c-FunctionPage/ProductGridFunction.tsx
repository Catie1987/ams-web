...
const ProductsGrid: FC<ProductsGridProps> = async ({functionName}) => {
    const t = await getTranslations('FunctionPage');
    const locale = await getLocale();
    const contentfulLocale = localeMap[locale as unknown as string] || 'en-US';
    const products = await getProductsbyFunction({
        functionName,
      ...
      <div className='w-full md:w-96 mt-10 mr-4 mb-4 relative'>
          <div className='border w-full rounded-lg flex flex-col shadow-lg p-4 sticky top-20 mb-10 md:mb-0'>
            ...
            <button className='mt-6 md:w-[340px] w-full border px-8 rounded-md py-6 overflow-hidden relative group bg-[--cta2] hover:bg-white hover:border-transparent hover:transition-all hover:duration-150 '>
                <a href={`${t('link1')}${encodeURIComponent(functionName || "")}`} className='w-full h-full flex items-center z-10 text-white'>
                  ...
                  <span className='mr-4 group-hover:text-[--cta2] transition-colors duration-300 text-start'>{`${t('button1')}${functionName}`}</span>
                  <ArrowRight/>
                </a>
            </button>
            <p className='mt-5'>
